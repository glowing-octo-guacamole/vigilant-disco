...
class LinkedBinaryTree(BinaryTree):
    ...
    def inorder_next(self, p):
        node = self._validate(p)
        # If there is a right child, the next node in inorder traversal is the leftmost node in the right subtree
        if node._right is not None:
            node = node._right
            while node._left is not None:
                node = node._left
            return self._make_position(node)
        # If p doesn't have a right child, traverse up the tree until a node that is the left child of its parent is found
    ...
    def postorder_next(self, p):
        node = self._validate(p)
        parent = node._parent
        # # If node is the root, there is no next node in postorder traversal
        if parent is None:
            return None
        # If p is the left child and the parent has a right child, the next node in the postorder traversal is the leftmost leaf node in the parent's right subtree
        if parent._right is not None and node == parent._left:
            ...
            while node._left is not None or node._right is not None:
                if node._left is not None:
                    ...
                    node = node._right
            return self._make_position(node)
        # If p is the right child or if the parent has no right child, the next node in the postorder traversal is the parent.
...
for position in positions:
    next_pos = tree.inorder_next(position)
...
for position in positions:
    next_pos = tree.postorder_next(position)
    if next_pos is not None:
        print(f"Node {position.element()} -> {next_pos.element()}")
    else:
        print(f"Node {position.element()} -> None")